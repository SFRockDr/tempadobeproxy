...
export default async function handler(req, res) {
  ...
  try {
    ...
    // If no contentElement found or too short, run a Readability fallback
    import { JSDOM } from 'jsdom';
    import { Readability } from '@mozilla/readability';

    // ...inside try, after scraping:
    if (!contentElement || contentElement.length === 0) {
    ...
    try {
        ...
        ['utm_source','utm_medium','utm_campaign','scid'].forEach(p => u.searchParams.delete(p));
        a.attr('href', u.toString());
    } catch {}
    });

    // Remove exact duplicate adjacent paragraphs (common after fragment merges)
